...
import { getServiceList, renewBalance, getTeamService } from '../../../api/service'
...
export default {
  ...
  head() {
    return {
      title: '服务续费 - EasyAPI',
      meta: [{ charset: 'utf-8' }, { hid: 'description', name: 'description', content: '服务续费' }, { hid: 'keyword', name: 'keyword', content: '服务续费' }]
    }
  },
  data() {
    return {
  ...
  methods: {
     ...
     */
    getServiceList() {
      getServiceList({ serviceId: this.service.serviceId }, this).then(res => {
        if (res.data.code === 1) {
      ...
      renewBalance(
        ...
        if (res.data.code === 1) {
          if (this.payment === '支付宝') {
            ...
            this.$confirm('请您在新打开的页面上完成充值。充值完成后，根据您的情况点击下面按钮。', '提示', {
            ...
            stompClient.connect({}, frame => {
              stompClient.subscribe('/topic/wxpay/' + this.team.user.id, message => {
