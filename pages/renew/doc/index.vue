	...
	<div>
		<div class="w-full h-auto">
			<div class="w-full h-20 bg-gray-50">
				<span v-if="item.state.user.userInfo.team"
			...
			<div class="renew_content max-w-screen-lg">
				...
				<div class="renew_fl">
					...
					<div class="">
						<el-input-number v-model="data.memberCount" controls-outside
							:min="nowMemberCount"></el-input-number>
					</div>
				</div>
		...
		<!-- 微信支付弹窗 -->
		<WeChatPay :visible.sync="data.wechatPayDialog" :weChatPayUrl="data.weChatPayUrl" :price="price" />
	</div>
</template>
...
function getTeamInfo(this: any) {
	...
	getTeamMoney({ teamId: teamId }, this).then((res: { data: { code: number; content: { balance: string; team: { endTime?: any; release?: any } } } }) => {
		if (res.data.code === 1) {
